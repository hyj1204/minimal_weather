  ...
  group('MetaWeatherApiClient', () {
    ...
    group('getWeather', () {
      ...
      test('makes correct http request', () async {
        ...
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        try {
          await metaWeatherApiClient.getWeatherList(locationId);
        } catch (_) {}
        verify(
          () => httpClient.get(
      ...
      test('throws WeatherRequestFailure on non-200 response', () async {
        ...
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        expect(
          () async => await metaWeatherApiClient.getWeatherList(locationId),
      ...
      test('returns weather on valid response', () async {
        ...
        ''');
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        final actual = await metaWeatherApiClient.getWeatherList(locationId);
        expect(
          actual.first,
          isA<Weather>()
